...
# plot figure with obervarions and model for water and sediment     
def plot_all(var_brom,var_water,var_sed1,var_sed2,title):    
    #ax1.cla()
    figure1 = plt.figure(figsize=(2,4), dpi=300)
#    figure1 = plt.figure(figsize=(5,8), dpi=400)	
#    figure1 = plt.figure(figsize=(3,7), dpi=400)
#    figure1 = plt.figure(figsize=(5,10), dpi=200)
#    figure1 = plt.figure(figsize=(3,6), dpi=200)
#    figure1 = plt.figure(figsize=(5,7), dpi=200)
    figure1.set_facecolor('white') # specify background color
    gs = gridspec.GridSpec(2,1)   # define 2 subplots, (i.e. y=2,x=1)
    gs.update(left = 0.2)         # shift figure from left boundary
    ax = figure1.add_subplot(gs[0]) 
    ax1 = figure1.add_subplot(gs[1])
    ax.set_ylim(310,0)
    ax1.set_ylim(3.1,-3.1)  #   ax1.set_ylim(5,-5) 
    water_ticks = np.arange(0, 310, 100)                                             
    swi_ticks = np.arange(-3, 3, 1.5)   
    ax.set_yticks(water_ticks)     
    ax1.set_yticks(swi_ticks)     
    ...
    plt.savefig(results_dir+title+'.eps', facecolor=figure1.get_facecolor(), edgecolor='none')
    #plt.show()
    plt.clf()
# ------------- FINALLY PLOT ! ------------
    
# plt.savefig("/home/dir/subdir/filename.png")   
plot_all(alk_brom,alk,alk199,alk200,'Alk')  
...
plot_all(tic_brom,tic,tic199,tic200,'DIC')
